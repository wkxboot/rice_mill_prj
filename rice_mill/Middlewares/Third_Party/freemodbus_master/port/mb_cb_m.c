...
#define APP_LOG_MODULE_NAME   "[callback]"
...
eMB_MASTER_ErrorCode eMBMasterRegInputCB( UCHAR * pucRegBuffer, USHORT usAddress, USHORT usNRegs )
{
    ...
    usAddress--;
    APP_LOG_INFO("read respose addr:%d size:%d\r\n",usAddress,usNRegs);
        ...
        USHORT usNRegs, eMB_MASTER_RegisterMode eMode)
{
            ...
            && (usAddress + usNRegs <= REG_HOLDING_START + REG_HOLDING_NREGS))
    {
        ...
        case MB_MASTER_REG_READ:
          APP_LOG_INFO("read holding reg addr:%d size:%d\r\n",usAddress,usNRegs);
            while (usNRegs > 0)
            {
        ...
        case MB_MASTER_REG_WRITE:
          APP_LOG_INFO("write respose addr:%d size:%d\r\n",usAddress,usNRegs);
            while (usNRegs > 0)
            {
